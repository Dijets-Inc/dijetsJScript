...
describe('PlatformVMAPI', () => {
  ...
  const username:string = 'DijetsInc';
  const password:string = 'password';

  const dijets:Dijets = new Dijets(ip, port, protocol, networkid, undefined, undefined, undefined, true);
  let api:PlatformVMAPI;
  let alias:string;

  const addrA:string = 'P-' + bech32.encode(dijets.getHRP(), bech32.toWords(bintools.cb58Decode("B6D4v1VtPYLbiUvYXtW4Px8oE9imC2vGW")));
  const addrB:string = 'P-' + bech32.encode(dijets.getHRP(), bech32.toWords(bintools.cb58Decode("P5wdRuZeaDt28eHMP5S3w9ZdoBfo7wuzF")));
  const addrC:string = 'P-' + bech32.encode(dijets.getHRP(), bech32.toWords(bintools.cb58Decode("6Y3kysjF9jnHnYkdS9yGAuoHyae2eNmeV")));

  beforeAll(() => {
    api = new PlatformVMAPI(dijets, '/ext/bc/P');
  ...
  test('refreshBlockchainID', async () => {
    ...
    let testAPI:PlatformVMAPI = new PlatformVMAPI(dijets, '/ext/bc/P');
  ...
  test("exportDJTX", async ()=>{
    let amount = new BN(100);
    let to = "abcdef";
    let username = "Robert";
    let password = "Paulson";
    let txID = "valid";
    let result:Promise<string> = api.exportDJTX(username, password, amount, to);
    let payload:object = {
        "result": {
            "txID": txID
        }
  ...
  test("importDJTX", async ()=>{
    let to = "abcdef";
    let username = "Robert";
    let password = "Paulson";
    let txID = "valid";
    let result:Promise<string> = api.importDJTX(username, password, to, blockchainid);
    let payload:object = {
        "result": {
            "txID": txID
        }
  ...
  describe('Transactions', () => {
    ...
    beforeEach(async () => {
      platformvm = new PlatformVMAPI(dijets, "/ext/bc/P");
      ...
      keymgr2 = new KeyChain(dijets.getHRP(), alias);
      keymgr3 = new KeyChain(dijets.getHRP(), alias);
      addrs1 = [];
      addrs2 = [];
      addrs3 = [];
      utxos = [];
      ...
      for (let i:number = 0; i < 4; i++) {
        ...
        const out:SECPTransferOutput = new SECPTransferOutput(ONEDJTX.mul(new BN(5)), addressbuffs, locktime, 1);
        const pout:ParseableOutput = new ParseableOutput(out);
        const lockout:StakeableLockOut = new StakeableLockOut(ONEDJTX.mul(new BN(5)), addressbuffs, locktime, 1, locktime.add(new BN(86400)), pout);
    ...
    test('buildImportTx', async () => {
      ...
      const txu2:UnsignedTx = set.buildImportTx(
        ...
        addrbuff3, addrbuff1, addrbuff2, [fungutxo], bintools.cb58Decode(PlatformChainID), platformvm.getTxFee(), await platformvm.getDJTXAssetID(), 
    ...
    test('buildExportTx', async () => {
      ...
      const txu1:UnsignedTx = await platformvm.buildExportTx(
        set, 
        amount,
        bintools.cb58Decode(Defaults.network[dijets.getNetworkID()].X["blockchainID"]),
        addrbuff3.map((a) => bintools.addressToString(dijets.getHRP(), "P", a)), 
        addrs1, 
        addrs2,
      ...
      const txu3:UnsignedTx = await platformvm.buildExportTx(
        set, amount, bintools.cb58Decode(Defaults.network[dijets.getNetworkID()].X["blockchainID"]), 
...
*/
    ...
    test('buildAddValidatorTx 3', async () => {
      ...
      let lockedBaseOut:SECPTransferOutput = new SECPTransferOutput(ONEDJTX.mul(new BN(2)), addrbuff1, locktime, 1);
      let lockedBaseXOut:ParseableOutput = new ParseableOutput(lockedBaseOut);
      let lockedOut:StakeableLockOut = new StakeableLockOut(ONEDJTX.mul(new BN(2)), addrbuff1, locktime, 1, locktime, lockedBaseXOut)
      ...
      let unlockedOut:SECPTransferOutput = new SECPTransferOutput(ONEDJTX.mul(new BN(2)), addrbuff1, locktime, 1);
