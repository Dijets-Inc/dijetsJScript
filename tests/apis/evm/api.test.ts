...
describe('EVMAPI', () => {
  ...
  const addrA: string = 'C-' + bech32.encode(dijets.getHRP(), bech32.toWords(bintools.cb58Decode("B6D4v1VtPYLbiUvYXtW4Px8oE9imC2vGW")));
  const addrB: string = 'C-' + bech32.encode(dijets.getHRP(), bech32.toWords(bintools.cb58Decode("P5wdRuZeaDt28eHMP5S3w9ZdoBfo7wuzF")));
  const addrC: string = 'C-' + bech32.encode(dijets.getHRP(), bech32.toWords(bintools.cb58Decode("6Y3kysjF9jnHnYkdS9yGAuoHyae2eNmeV")));

  beforeAll(() => {
    api = new EVMAPI(dijets, '/ext/bc/C/djtx', blockchainid);
  });
  ...
  test('refreshBlockchainID', async () => {
    ...
    const testAPI: EVMAPI = new EVMAPI(dijets, '/ext/bc/C/djtx', n5bcID);
