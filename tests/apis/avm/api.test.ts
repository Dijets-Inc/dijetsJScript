...
describe('AVMAPI', () => {
  ...
  const addrA:string = 'X-' + bech32.encode(dijets.getHRP(), bech32.toWords(bintools.cb58Decode("B6D4v1VtPYLbiUvYXtW4Px8oE9imC2vGW")));
  const addrB:string = 'X-' + bech32.encode(dijets.getHRP(), bech32.toWords(bintools.cb58Decode("P5wdRuZeaDt28eHMP5S3w9ZdoBfo7wuzF")));
  const addrC:string = 'X-' + bech32.encode(dijets.getHRP(), bech32.toWords(bintools.cb58Decode("6Y3kysjF9jnHnYkdS9yGAuoHyae2eNmeV")));

  beforeAll(() => {
    api = new AVMAPI(dijets, '/ext/bc/X', blockchainid);
  ...
  test('refreshBlockchainID', async () => {
    ...
    let testAPI:AVMAPI = new AVMAPI(dijets, '/ext/bc/avm', n3bcID);
...
test("import", async ()=>{
  ...
  test('createFixedCapAsset', async () => {
    const kp:KeyPair = new KeyPair(dijets.getHRP(), alias);
  ...
  describe('Transactions', () => {
    ...
    beforeEach(async () => {
      avm = new AVMAPI(dijets, "/ext/bc/X", blockchainid);
    ...
    test('buildCreateNFTAssetTx', async () => {
      ...
      let txu2:UnsignedTx = set.buildCreateNFTAssetTx(
          dijets.getNetworkID(), bintools.cb58Decode(avm.getBlockchainID()), 
    ...
    test('buildCreateNFTMintTx', async () => {
      ...
      let payload:Buffer = Buffer.from("Dijets");
    ...
    */
    ...
    test('buildImportTx', async () => {
      ...
      const txu2:UnsignedTx = set.buildImportTx(
        ...
        addrbuff3, addrbuff1, addrbuff2, [fungutxo], bintools.cb58Decode(PlatformChainID), avm.getTxFee(), await avm.getDJTXAssetID(), 
