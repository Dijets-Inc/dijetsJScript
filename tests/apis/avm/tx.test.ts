...
describe('Transactions', () => {
  ...
  const memo:Buffer = Buffer.from("DijetsJS");
  ...
  beforeAll(async () => {
    dijets = new Dijets(ip, port, protocol, netid, undefined, undefined, null, true);
    api = new AVMAPI(dijets, '/ext/bc/avm', blockchainid);
  ...
  beforeEach(() => {
    set = new UTXOSet();
    keymgr1 = new KeyChain(dijets.getHRP(), alias);
  ...
  test('Create small BaseTx that is Goose Egg Tx', async () => {
    ...
    const transferableOutput:TransferableOutput = new TransferableOutput(djtxAssetID, output);
    ...
    const transferableInput:TransferableInput = new TransferableInput(txid, outputIndex, djtxAssetID, input);
  ...
  test('Creation UnsignedTx Check Amount', () => {
    expect(() => {
      set.buildBaseTx(
        netid, blockchainID,
        ONEDJTX.mul(new BN(amnt * 10000)), assetID,
  ...
  test('Creation Tx3 using OperationTx', () => {
    const txu:UnsignedTx = set.buildNFTTransferTx(
      netid, blockchainID, 
      addrs3, addrs1, addrs2, nftutxoids, new BN(90), djtxAssetID, undefined,
      UnixNow(), UnixNow().add(new BN(50)), 1,
    );
    const tx:Tx = txu.sign(keymgr1);
    const tx2:Tx = new Tx();
