...
import { Defaults, PlatformChainID, PrimaryAssetAlias, ONEDJTX } from '../../utils/constants';
...
export class AVMAPI extends JRPCAPI {
  ...
  checkGooseEgg = async (utx:UnsignedTx, outTotal:BN = new BN(0)): Promise<boolean> => {
    ...
    let outputTotal:BN = outTotal.gt(new BN(0)) ? outTotal : utx.getOutputTotal(djtxAssetID);
  ...
  /**
     ...
     * @returns String representing the transaction id
     */
  exportDJTX = async (username:string, password:string, to:string, amount:BN):Promise<string> => {
    const params:any = {
      to,
      amount: amount.toString(10),
      username,
    ...
    return this.callMethod('avm.exportDJTX', params).then((response:RequestResponseData) => response.data.result.txID);
  };
  ...
  /**
     ...
     * @returns String representing the transaction id
     */
  importDJTX = async (username:string, password:string, to:string, sourceChain:string):Promise<string> => {
    const params:any = {
      to,
      sourceChain,
      username,
    ...
    return this.callMethod('avm.importDJTX', params).then((response:RequestResponseData) => response.data.result.txID);
  };
  ...
  buildSECPMintTx = async (
    ...
    let djtxAssetID:Buffer = await this.getDJTXAssetID();
  ...
  */
   ...
   */
  constructor(core:DijetsCore, baseurl:string = '/ext/bc/X', blockchainID:string = '') {
    super(core, baseurl);
    this.blockchainID = blockchainID;
